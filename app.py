...
def create_prophet_plot(symbol):
    ...
    df_prophet = df_prophet[df_prophet['ds'] >= (df_prophet['ds'].max() - timedelta(days=30))]
    prophet_forecast = prophet_forecast[prophet_forecast['ds'] >= (prophet_forecast['ds'].max() - timedelta(days=31))]
...
def training_model(data):
    ...
    ts=24
    X = create_dataset(dataset, time_step=ts)
    ...
    model.fit(X, dataset[ts:], epochs=10, batch_size=16, verbose=0)
...
def historical_plot():
    ...
    df = df[df['Datetime'] >= (df['Datetime'].max() - timedelta(days=30))]
    ...
    future_dates = pd.date_range(start=data['Datetime'].iloc[-1] + pd.Timedelta(hours=1), periods=24, freq='1h')
